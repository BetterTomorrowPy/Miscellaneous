...
def save_data(data=None):
    ...
    if data:
        ...
        db.execute(sql, data.get('country', ''), data.get('area', ''), data.get('name', ''), data.get('logo', ''), 
            ...
            str(data.get('level', '')), data.get('type', ''), data.get('xz', ''))
    return 


def update_part_info():
    """"""
    # sql = "UPDATE xiongyang.t_institution_source SET f_country_id = %s WHERE f_site_id = 27 AND f_country = %s"
    # db.execute(sql, '4165', '英国')
    # sql = "SELECT id, ins_name, ins_ename, ins_cname, f_site_url FROM xiongyang.t_institution_source WHERE f_level_id < 3"
    sql = "SELECT id, ins_name FROM xiongyang.t_institution_source where ins_cname like '%%研究%%'"
    rows = db.query(sql)
    for row in rows:
        # print(row)
        # url = row.get('f_site_url', '')
        # if url:
        #     if 'college' in url.lower():
        db.execute("UPDATE xiongyang.t_institution_source SET f_level_id = 4 WHERE id = %s", row.get('id'))
    print(len(rows))
...
=======
    main()
    # db.execute("DELETE FROM xiongyang.t_institution_source WHERE f_site_id = 27")
>>>>>>> 894d82b1d7fab3f956f0bc6481f9d863ea4a16a2
